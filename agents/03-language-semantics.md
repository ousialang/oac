...
- Comptime still fails closed on runtime-only operations the evaluator cannot model, including extern calls, pointer load/store builtins, and transitive helper paths that require those operations; `print(...)` and `print_str(...)` are the explicit output-only exceptions and are treated as ignored debug sinks during comptime.
