...
- `String.make_bytes` normalizes `Bytes.len` fail-closed (`len < 0` clamps to `0`) and is used by `String.bytes`, `String.from_literal_parts`, and `String.from_heap_parts` so `Bytes` payloads returned at call sites preserve the invariant.
