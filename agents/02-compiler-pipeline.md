...
6. Execute comptime applies with `comptime::execute_comptime_applies`.
   ...
   - `comptime apply` targets still must be declared as `comptime fun`, but those functions may now call imported user helpers, stdlib semantic wrappers, ordinary pure helpers with `pre` blocks, receiver methods, operator-trait lowered helper paths, `match`, struct literals/field access, enum payload matching, recursion (fuel-limited), and debug-output sinks `print(...)` / `print_str(...)` (discarded output, deterministic `0` return); other runtime-only operations that the evaluator cannot model still fail closed.
