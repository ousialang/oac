...
- `crates/oac/src/flat_imports.rs` also includes a regression that imported comptime helper files can derive declarations before resolve (flat-import merge happens before `execute_comptime_applies`).
...
- `crates/oac/src/ir.rs` also includes comptime enum reflection/emission builtin coverage (`is_enum`, `as_enum_opt`, `enum_variant_count`, `enum_variant_at`, `variant_name`, `variant_payload_type_opt`, `declset_add_empty_enum`, `declset_add_enum_tag_variant`, `declset_add_enum_payload_variant`) plus runtime rejection coverage for both enum semantic introspection and emission builtins.
