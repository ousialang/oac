...
- `crates/oac/src/comptime.rs` also covers the bootstrap-resolved comptime execution surface: receiver-style semantic option chains, `Option.unwrap()`, `String.concat()`, `match`, struct literals/field access, ordinary pure helper calls, comptime preconditions, custom operator-impl lowering through normalized resolved calls, recursion/fuel limits, multi-apply rebuild visibility, and `Meta.expr_opt(...).definition_location_opt()` source metadata from resolved expression ids/spans.
...
- `crates/oac/src/integer_safety.rs` includes unit coverage for reachable integer-site collection plus shared checker pruning that drops post-site-only callees, `crates/oac/src/verification.rs` includes cache-summary coverage that ordinary summaries now include integer-safety obligations, and `crates/oac/src/comptime.rs` includes checked-`I32` overflow/divide-by-zero regressions plus enum reflection/emission execution coverage (including derive-tags and payload-preserving enum cloning).
