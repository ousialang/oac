...
int main(int argc, char* argv[]) {
	static const int8_t no_more_options = -1;
	...
	int parsed_option = getopt_long(argc, argv, options_synopsis, options, &current_argv_index);
	if (parsed_option == no_more_options) print_human_readable_version();
	return EX_OK;
	...
	do {
		...
		parsed_option = getopt_long(argc, argv, options_synopsis, options, &current_argv_index);
	} while (parsed_option != no_more_options);
...
void print_unknown_options_warning(char* argv[], std::vector<int> unknown_options_by_argv_index) {
	...
	for (int& i : unknown_options_by_argv_index) { std::cout << " " << argv[i]; }
	std::cout << std::endl;
}
...
void print_human_readable_version() {
	...
	std::cout << "Ousia " << version_major << "." << version_minor << "." << version_patch << "-"
	          << hyphenated_version_tags << " (" << version_build << ")" << std::endl;
}
