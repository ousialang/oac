...
token_t** tokens_from_string(char source[]) {
	...
	while (index_char != length) {
		...
		if (isalpha(source[index_char])) {
			...
			tokens[index_current_token]
			        = &(token_t){ WORD, &(token_info_t){}, index_lexeme,
			...
			tokens[index_current_token]
			        = &(token_t){ WORD, &(token_info_t){}, index_lexeme,
				                  index_char };
			tokens[index_current_token]
			        = &(token_t){ WORD, &info, index_lexeme, index_char };
			do {
				index_char++;
			} while (!isalpha(source[index_char]));
			...
			tokens[index_current_token] = &(token_t){
				WORD, &(token_info_t){ .integer = (token_integer_t){ 10 } },
				index_lexeme, index_char - index_lexeme
			};
			// token.info =
			token.length = index_char - index_lexeme;
		} else if (source[index_char] == '(') {
