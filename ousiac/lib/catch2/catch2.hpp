...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                     \
	...
	} while (Catch::isTrue(false && static_cast<bool>(!!(__VA_ARGS__))))  // the expression here is
	                                                                      // never evaluated at
...
AutoReg::AutoReg(ITestInvoker* invoker,
                 ...
                 NameAndTags const& nameAndTags) noexcept {
	try {
		getMutableRegistryHub().registerTest(makeTestCase(invoker, extractClassName(classOrMethod),
		                                                  nameAndTags.name, nameAndTags.tags,
		                                                  lineInfo));
	} catch (...) {
		// Do not throw when constructing global objects, instead register the exception to be
		// processed later
