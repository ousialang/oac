...
- Prove and struct-invariant CHC solving keeps baseline two-step retries (`10s`, `30s`) and optionally adds a third attempt for candidate profile only when the SMT obligation is large and still `unknown` after retry.
...
- Unknown diagnostics now include solver-attempt ladders (`attempts=[...]`) with timeout/result/reason snippets in both prove and struct-invariant failures.
...
- Verification profile/policy plumbing lives in `crates/oac/src/verification_profile.rs` and `crates/oac/src/verification_solver.rs`; outcome capture/comparison helpers live in `crates/oac/src/verification_outcomes.rs`.
