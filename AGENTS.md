...
- Non-extern, non-comptime functions may declare an optional multiline precondition block between the return type and body: `fun f(x: I32) -> I32 pre { x > 5 } { ... }`. `pre` blocks are verification-only, require at least one Bool clause, and are rejected on `extern fun`, `comptime fun`, and trait method declarations.
...
- The stdlib entrypoint `crates/oac/src/std/std.oa` is now an import aggregator over split files in `crates/oac/src/std/` (`std_ascii.oa`, `std_char.oa`, `std_null.oa`, `std_comptime.oa`, `std_option_result.oa`, `std_string.oa`, `std_ref.oa`, `std_collections.oa`, `std_set.oa`, `std_vec.oa`, `std_traits.oa`, `std_json.oa`, `std_clib.oa`, `std_io.oa`).
...
- Comptime semantic reflection builtins over `Type` now cover both structs and enums: `is_struct` / `as_struct_opt` / `struct_field_count` / `struct_field_at` / `field_name` / `field_type` and `is_enum` / `as_enum_opt` / `enum_variant_count` / `enum_variant_at` / `variant_name` / `variant_payload_type_opt`.
- Comptime semantic emission builtins on `DeclSet` now cover both struct/invariant emission and incremental enum construction: `declset_new`, `declset_add_empty_enum`, `declset_add_enum_tag_variant`, `declset_add_enum_payload_variant`, `declset_add_derived_struct`, and `declset_add_invariant_field_gt_i32`; all semantic introspection/emission helpers, along with `type_name`, `resolve_type`, `expr_meta_opt`, `definition_location_opt`, `is_some`, `unwrap`, and `concat`, remain comptime-only and are rejected from runtime functions.
- The comptime evaluator now preloads stdlib comptime helpers/types before user `comptime apply` execution, so std helpers such as `derive_enum_tags(T)` in `crates/oac/src/std/std_comptime.oa` are callable from user source; imported same-directory user helpers remain available through flat-import merging before comptime execution.
