...
- Prove and struct-invariant CHC solving now retries once with a longer timeout when Z3 returns `unknown` (first pass `10s`, retry `30s`) before surfacing fail-closed errors.
