...
- During `oac build`, proof obligations are orchestrated by `crates/oac/src/verification.rs` over a shared reachable-function/QBE-function-map context.
- The orchestrator runs prove obligations first at reachable `prove(...)` sites by synthesizing per-site QBE checker functions that return `1` on violated proof conditions and `0` on success (`unsat` passes, `sat` fails), then short-circuits on failure.
- If the prove batch passes, the same orchestrator runs struct-invariant obligations at user-function call return sites (reachable from `main`) by synthesizing per-site QBE checker functions from compiled QBE: the target call site is instrumented with an invariant check and checker exit is `1` on violation / `0` on success (`unsat` passes, `sat` fails).
