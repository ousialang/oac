...
(define-function subr_apply_expr (args env)
  (let ((fun (k_car args)))
    (and (is <expr> fun)
	 (let ()
	   (let* ((arguments (k_cdr args))
		  (argl      arguments)
		 ...
		 (or (is <pair> argl) (k_error "too few arguments: ("fun" "arguments")"))
		 ...
		 (set argl    (get_tail argl)))
	       (and (is <symbol> formals)
		    (let ()
		      ...
		      (set env (new-<pair> tmp env))
		      (set argl ())))
	       (and argl (k_error "too many arguments: ("fun" "arguments")"))
	       (set defn (get_tail defn))
	       (while (is <pair> defn)
		 (set argl (k_eval (get_head defn) env))
		 (set defn (get_tail defn)))
	       argl))))))

(define-function subr_apply_fixed (args env)
  (let ((fun  (k_car args))
	(argl (k_cdr args)))
    (and (is <fixed> fun)
	 (k_apply (get <fixed> function fun) argl env))))

(define-function subr_apply (args env)
  (k_apply (k_car args) (k_cadr args) (or (k_caddr args) env)))

(define-function subr_eval_symbol (args env)
  (let ((exp (k_car args))
	(val (k_assq exp env)))
    (or (is <pair> val) (fatal1 "undefined variable: %s" (if (is <symbol> exp) (get <symbol> _bits exp) "(non-<symbol>)")))
    (get_tail val)))
...
(define-function subr_eval_pair (args env)
  (let ((exp  (k_car args))
	(head (k_eval (k_car exp) env)))
    (gc-protect (head)
      (k_set_array_at trace_stack trace_depth exp)
      (set trace_depth (+ trace_depth 1))
