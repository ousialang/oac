...
#define GC_PROTECT(V)                                                          \
  struct GC_StackRoot _sr_##V;                                                 \
  _sr_##V.root = (void *)&V;                                                   \
  GC_push_root(&_sr_##V, #V, __FILE__, __LINE__)
...
GC_API inline void GC_push_root(struct GC_StackRoot *sr, const char *name,
...
static int GC_roots_include(struct GC_StackRoot *roots,
...
GC_API inline void GC_pop_root(struct GC_StackRoot *sr, const char *name,
                               const char *file, int line) {
  struct GC_StackRoot *nr = sr->next;
...
static void buffer_append(struct buffer *b, int c) {
  if (b->position == b->size)
    b->buffer =
        b->buffer ? realloc(b->buffer, b->size *= 2) : malloc(b->size = 32);
...
static inline oop _checkType(oop obj, int type, char *name, char *file,
                             int line) {
...
#define set(OBJ, TYPE, FIELD, VALUE)                                           \
  (checkType(OBJ, TYPE)->TYPE.FIELD = (VALUE))
...
static oop s_set = nil, s_quote = nil, s_lambda = nil, s_let = nil,
           s_quasiquote = nil, s_unquote = nil, s_unquote_splicing = nil,
           s_t = nil, s_dot = nil;
static oop f_lambda = nil, f_let = nil, f_quote = nil, f_set = nil;
static oop globals = nil, expanders = nil, encoders = nil, evaluators = nil,
           applicators = nil;
...
static oop _newString(size_t len) {
  char *gstr = GC_malloc_atomic(len + 1);
...
static oop newSymbol(char *cstr) {
  oop obj = newBits(Symbol);
  set(obj, Symbol, bits, strdup(cstr));
  return obj;
}
...
static oop newArray(int tally) {
  oop elts = _newOops(_Array, sizeof(oop) * tally);
  GC_PROTECT(elts);
...
static oop newExpr(oop defn, oop env) {
  ...
  set(obj, Expr, env, env);
  return obj;
}
static oop newForm(oop function) {
  oop obj = newOops(Form);
  set(obj, Form, function, function);
  return obj;
}
...
static oop intern(char *cstr) {
  ...
  for (list = symbols; is(Pair, list); list = getTail(list)) {
    ...
    if (!strcmp(cstr, get(sym, Symbol, bits)))
      return sym;
  }
  oop sym = nil;
  GC_PROTECT(sym);
...
static int isDigit10(int c) {
  return 0 <= c && c <= 127 && (CHAR_DIGIT10 & chartab[c]);
}
static int isDigit16(int c) {
...
static int isOctal(int c) { return '0' <= c && c <= '7'; }
...
static int readChar(int c, FILE *fp) {
  if ('\\' == c) {
    ...
    case 'u': {
      ...
      return (digitValue(a) << 24) + (digitValue(b) << 16) +
             (digitValue(c) << 8) + digitValue(d);
    }
    case 'x': {
      int x = 0;
...
static oop read(FILE *fp) {
  for (;;) {
    ...
    default: {
      ...
      fatal(isPrint(c) ? "illegal character: 0x%02x '%c'"
...
static oop define(oop name, oop value, oop env) {
  oop ass;
  ass = newPair(name, value);
  GC_PROTECT(ass);
  oop ent = newPair(ass, getTail(env));
  GC_UNPROTECT(ass);
  setTail(env, ent);
  return ass;
}
...
static oop concat(oop head, oop tail) {
  ...
  GC_PROTECT(tail);
  head = newPair(getHead(head), tail);
  GC_UNPROTECT(tail);
  return head;
}
...
static void fatal(char *reason, ...) {
  ...
  if (nil != cdr(backtrace)) {
    ...
    apply(cdr(backtrace), args, globals);
    GC_UNPROTECT(args);
  } else {
    int i = traceDepth;
    while (i--) {
...
static oop apply(oop fun, oop arguments, oop env) {
  ...
  if (nil != ap) {
    oop args = newPair(fun, arguments);
    GC_PROTECT(args);
...
static subr(quote) { return car(args); }

static subr(lambda) { return newExpr(args, env); }
...
static subr(define) {
  ...
  GC_PROTECT(value);
  define(symbol, value, globals);
  GC_UNPROTECT(value);
  return value;
}

#define _do_unary() _do(com, ~)
...
#define _do_binary()                                                           \
  _do(add, +) _do(mul, *) _do(div, /) _do(mod, %) _do(bitand, &) _do(bitor, |) \
      _do(bitxor, ^) _do(shl, <<) _do(shr, >>)
...
#undef _do
  ...
  switch (getType(lhs)) {
  case Long:
    ans = (is(Long, rhs) && (getLong(lhs) == getLong(rhs)));
    break;
  case String:
    ans = (is(String, rhs) &&
           !strcmp(get(lhs, String, bits), get(rhs, String, bits)));
    break;
  default:
    ans = (lhs == rhs);
    break;
...
static subr(current_environment) { return env; }
...
static subr(read) {
  ...
  if (!stream)
    return nil;
  oop head = newPair(nil, nil), tail = head;
  GC_PROTECT(head);
  oop obj = nil;
  GC_PROTECT(obj);
  for (;;) {
...
static subr(eval_symbol) {
  ...
  if (!is(Pair, val))
    fatal("undefined variable: %s",
          is(Symbol, obj) ? get(obj, Symbol, bits) : "<non-symbol>");
  return getTail(val);
}
...
static subr(eval_pair) {
  ...
  if (is(Fixed, head))
    ...
    oop argl = evlist(getTail(obj), env);
    GC_PROTECT(argl);
    head = apply(head, argl, env);
    GC_UNPROTECT(argl);
  }
  GC_UNPROTECT(head);
  --traceDepth;
...
static subr(arrayP) { return is(Array, car(args)) ? s_t : nil; }
...
static void sigint(int signo) { fatal("\nInterrupt"); }
...
int main(int argc, char **argv) {
  ...
  tmp = newArray(32);
  expanders = define(intern("*expanders*"), tmp, globals);
  tmp = newArray(32);
  encoders = define(intern("*encoders*"), tmp, globals);
  tmp = newArray(32);
  evaluators = define(intern("*evaluators*"), tmp, globals);
  tmp = newArray(32);
  applicators = define(intern("*applicators*"), tmp, globals);

  arrayAtPut(getTail(evaluators), Symbol,
             newSubr(subr_eval_symbol, "eval-symbol"));
  arrayAtPut(getTail(evaluators), Pair, newSubr(subr_eval_pair, "eval-pair"));

  arrayAtPut(getTail(applicators), Fixed,
             newSubr(subr_apply_fixed, "apply-fixed"));
...
#define _do(NAME, OP)                                                          \
  tmp = newSubr(subr_##NAME, #OP);                                             \
  define(intern(#OP), tmp, globals);
  _do_unary();
  _do_binary();
  _do(sub, -);
  _do_relation();
#undef _do
  ...
  f_set = cdr(assq(s_set, globals));
  GC_add_root(&f_set);
  f_quote = cdr(assq(s_quote, globals));
  GC_add_root(&f_quote);
  f_lambda = cdr(assq(s_lambda, globals));
  GC_add_root(&f_lambda);
  f_let = cdr(assq(s_let, globals));
  GC_add_root(&f_let);
