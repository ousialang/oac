...
static const std::string github_zipball_url = "https://github.com/%s/%s/archive/%s.zip";
static const std::string gitlab_zipball_url = "https://gitlab.org/%s/%s/repository/archive.zip?ref=%s";
...
#define URL_SOURCEFORGE_ZIPBALL "https://downloads.sourceforge.net/project/%s/%s/%s/%s-%s.zip"
...
static struct option options[]
        = { { "version", required_argument, 0, 'v' }, { "branch", required_argument, 0, 'b' },
	    { 0, 0, 0, 0 } };

static const std::string options_synopsis = "v:b:";

int install_main(int argc, char* argv[]) {
	int argv_current_index = 0;
	int opt;
	struct parameter params[argc];
	while (opt = getopt_long(argc, argv, options_synopsis, options, &argv_current_index), opt != -1) {
